...
import { z } from "zod";
import { registerPatientAccount, createPatientSession, PatientAuthError } from "@/lib/server/patientAuthStore";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const account = registerPatientAccount(input);
    ...
    if (err instanceof PatientAuthError) return jsonError(err.message, 400);
